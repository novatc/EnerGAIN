...
def plot_reward(reward_log: list, window_size: int, model_name: str):
    ...
    plt.title('Reward über die Zeit', fontsize=14)
...
def plot_savings(savings_log: list, window_size: int, model_name: str):
    ...
    plt.title('Kapital über die Zeit', fontsize=14)
...
def plot_charge(window_size: int, battery, model_name: str):
    ...
    plt.title('SOC über die Zeit', fontsize=14)
    plt.xlabel('Anzahl der Handelssignale', fontsize=12)
...
def plot_holding(holding_logs: list, model_name: str, da_data: pd.DataFrame):
    ...
    plt.title('Halten über die Zeit', fontsize=14)
    plt.ylabel('Preis (€/kWh)', fontsize=12)
    plt.xlabel('Schritt', fontsize=12)
    plt.legend(fontsize=12)
    plt.tight_layout()
...
def kernel_density_estimation(trade_list: list, model_name: str, da_data: pd.DataFrame):
    ...
    plt.figure(figsize=(14, 7))
    plt.plot(x_vals, generated_density(x_vals), label='Generated Prices', color='blue')
    plt.plot(x_vals, historic_density(x_vals), label='Historic Prices', color='orange')
    ...
    plt.title('Kerndichteschätzung von generierten und historischen Preisen', fontsize=14)
...
def plot_soc_and_boundaries(soc_log, upper_bound_log, lower_bound_log, model_name: str):
    ...
    plt.title('SOC und Flexibilitätsgrenzen über die Zeit', fontsize=14)
    plt.xlabel('Schritt', fontsize=12)
    plt.ylabel('SOC', fontsize=12)
    plt.legend(fontsize=12)
    plt.grid(True)
