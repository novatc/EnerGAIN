...
from envs.assets.features import clip_to_battery, clip_to_budget
from envs.assets.plot_engien import *


class BaseState(gym.Env):
    """
    BaseEnv plus the agent's own state.

    BaseEnv rejects trades on battery headroom and affordability but shows the agent neither, so
    it bids blind on the quantities that decide validity: 5860 of its 6362 rejected trades on the
    average year carried the 'battery' label. This variant appends [soc, savings] to the
    observation and clips the day-ahead amount into what the battery and the capital can take,
    the way the multi-market envs already clip against the PRL flexibility band. Under a random
    policy that removes every infeasible-trade rejection (4221 -> 0); what remains is 'market
    rejected', which is a genuine market outcome.
    """
    ...
    def __init__(self, da_data_path: str, validation=False):
        ...
        # +2 for the agent's own state: soc and savings
        obs_low = np.append(low_boundary, [0.0, -np.inf])
        obs_high = np.append(high_boundary, [self.battery.capacity, np.inf])
        self.observation_space = spaces.Box(low=obs_low, high=obs_high,
                                            shape=(self.da_dataframe.shape[1] + 2,))
    ...
    def get_observation(self):
        """
        Returns the current state of the environment, market row plus the agent's own state.

        :return: the current state of the environment as a numpy array
        """
        observation = self.da_dataframe.iloc[self.day_ahead.get_current_step()].to_numpy(dtype=float)
    ...
    def render(self, mode='human'):
        ...
        :return:
        """
        plot_savings(self.trade_log, 'base_state')
        plot_savings_on_trade_steps(trade_log=self.trade_log, total_steps=self.da_dataframe.shape[0],
                                    model_name='base_state')
        plot_charge(self.battery, 'base_state')
        plot_trades_timeline(trade_source=self.trade_log, title='Trades', buy_color='green', sell_color='red',
                             model_name='base_state', data=self.da_dataframe, plot_name='trades')
        plot_trades_timeline(trade_source=self.invalid_trades, title='Invalid Trades', buy_color='black',
                             sell_color='brown', model_name='base_state', data=self.da_dataframe,
                             plot_name='invalid_trades')
        plot_holding(self.holding, 'base_state', da_data=self.da_dataframe)
        kernel_density_estimation(self.trade_log, 'base_state', da_data=self.da_dataframe)
